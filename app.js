...
if(state.contentSeedVersion!==1){
  if(!state.topics.length){state.topics=['Fisiologia — Ciclo cardíaco','Clínica Médica — DM','Clínica Médica — TEP','Clínica Médica — Pneumonia','Clínica Médica — Asma'];state.commitments=[{id:'starter-internship',dateIndex:0,title:'Estágio'}];state.dailyLimits=studyDates.map(()=>2);shouldSeedSchedule=true}
  ...
  state.contentSeedVersion=1;
}
if(state.scheduleVersion!==3){state.sessions=[];state.unscheduledTopics=0;state.scheduleVersion=3;localStorage.setItem(STORAGE_KEY,JSON.stringify(state))}
...
function generateSchedule(){
  ...
  studied.forEach((session,order)=>{
    const preferred=session.dateIndex+(order%2===0?1:2);let dateIndex=-1;
  ...
  state.sessions=sessions;state.unscheduledTopics=unscheduled;state.scheduleVersion=3;save();renderSchedule();
